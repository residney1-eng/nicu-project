...
from noise_sensor import get_average_noise 
from config import (
    ALARM_THRESHOLD_DB,
    ALARM_RESET_DB,
    OUTSIDE_MIC_DEVICE,
...
def get_anc_db():
    ...
    try:
        ...
        sock.close()
        last_anc_db = float(data.decode().strip())
        return last_anc_db  # float(data.decode().strip())
...
class Dashboard(QWidget):
    ...
    def update_dashboard(self):
        try:
            outside_noise = get_average_noise(OUTSIDE_MIC_DEVICE,offset=offset_db_out)
            inside_noise = get_anc_db()
            ...
            if self.anc_enabled and noise_diff > 3:
                print(f"outside={outside_noise} inside={inside_noise}")

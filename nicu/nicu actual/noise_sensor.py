...
def get_noise_level(device, offset=offset_db):
    ...
    db = 20 * np.log10(rms) + offset
    db = max(0, min(db, 120))
    return round(db, 1)


def get_average_noise(device_list, offset=offset_db):
    readings = []

    for device in device_list:
        try:
            db = get_noise_level(device, offset=offset)
            readings.append(db)
